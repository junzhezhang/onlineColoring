...
void* SmartMemPool::Malloc(size_t size){
    ...
    if(mallocFlag==0){
        ...
        if ((Table_r2Ver.find(lookupIdx)->second.size == size)&& (Table_r2Ver.find(lookupIdx)->second.Occupied==0)){((gc-location)%maxLen == Table_r2Ver.find(resp_rIdx)->second.d_idx) && (ptr == Table_r2Ver.find(resp_rIdx)->second.ptr) &&(Table_r2Ver.find(resp_rIdx)->second.Occupied ==1))
