...
pair<size_t,size_t> FFallocation(vector<pair<size_t,size_t>> colorMerged,size_t size, size_t local_offset){
    ...
    return pair<size_t,size_t>(yLocation+local_offset,yLocation+size-1+local_offset);
...
void* SmartMemPool::Malloc(size_t size){
    //verifySection, size, only 2 items larger than this.
//    if(size>14000000){
//        cout<<"size is large: "<<size/1000000<<endl;
//    }
    ...
    if (gc == globeCounter){
        ...
        //here to verify if the coloring got overlapping.
...
//        cout<<"============print tables done==========="<<endl;
    ...
    if(mallocFlag==0){
        ...
        cout<<"now flag changed to color-malloc"<<endl;//TODO(junzhe) remove
        ...
        if ((Table_r2Ver.find(lookupIdx)->second.size == (size_t) size)&& (Table_r2Ver.find(lookupIdx)->second.Occupied==0)){
            ...
            cout<<"coming to this section"<<endl;
            //mallocFlag = 0;
            allocatedPtr = malloc(size);
            cout<<"condition M3"<<endl;
            loadLog[gc]=make_pair(loadLog.find(gc-1)->second.first+size,loadLog.find(gc-1)->second.second);
        }
    }
    ...
    cout<<gc<<' '<<allocatedPtr<<' '<<size<<endl;
    //verify
    if(Table_p2s.find(allocatedPtr)->second>14000000){
        cout<<"size is large in build: "<<size/1000000<<' '<<Table_p2s.find(allocatedPtr)->second<<endl;
    }
...
void SmartMemPool::Free(void* ptr){
    ...
    cout<<"maxLen, location and flag: "<<maxLen<<' '<<location<<' '<<mallocFlag<<endl;
    cout<<" offset and maxload: "<<offset<<" "<<maxload<<endl;
    size_t sizeDeallocate=0;
    if (!(Table_p2s.find(ptr)==Table_p2s.end())){
        sizeDeallocate = Table_p2s.find(ptr)->second;
        Table_p2s.erase(ptr);
    }else{
    ...
    //verify
    if (sizeDeallocate>14000000) {cout<<"size is large: "<<ptr<<' '<<sizeDeallocate/1000000<<endl;}
    ...
    if ((globeCounter==-1)||(gc<globeCounter)){
        ...
        void* verifyPtr = (void*)((char*)ptrPool+Table_r2Ver.find(resp_rIdx)->second.offset*sizeof(char));
        cout<<"pair of r and d are "<<resp_rIdx<<" "<<lookupIdx<<endl;
        if((Table_r2Ver.find(resp_rIdx)->second.Occupied==1)&&(verifyPtr==ptr)){
            ...
            cout<<"condition F2 "<<sizeDeallocate/1000000<<endl; //TODO(junzhe) 130 only.
...
unsigned long SmartMemPool::getMaxLoad(string loadFlag){
    ...
    if (loadFlag=="cuda"){
        ...
        for (int i=0; i<loadLog.size();i++){
            cudaLoadLog.push_back(loadLog.find(i)->second.first);
        }
...
//        cout<<cudaLoadLog[277]<<endl;
    ...
    if (loadFlag=="memPool"){
        ...
        for (int i=0; i<loadLog.size();i++){
            colorLoadLog.push_back(loadLog.find(i)->second.second);
        }
        ...
        cout<<colorLoadLog[823]<<endl;
        cout<<colorLoadLog[824]<<endl;
        cout<<colorLoadLog[825]<<endl;
        cout<<colorLoadLog[1012]<<endl;
    }
    
    if (loadFlag=="print"){
        for (int i=0; i<loadLog.size();i++){
            cout<<i<<' '<<loadLog.find(i)->second.first/1000000<<" "<<loadLog.find(i)->second.second/1000000<<endl;
        }
