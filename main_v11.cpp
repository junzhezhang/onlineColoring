...
vector<pair<size_t, size_t>>  mergeSeg(vector<pair<size_t,size_t>> colorOccupied){
    ...
    if(colorOccupied.size()<=1){
        //verify
        cout<<"condition ms1, no need to merge"<<endl;
    ...
    while (m<(colorOccupied.size()-1)){
        //verify
        //cout<<"colorOccupied size: "<<colorOccupied.size()<<endl;
        if ((colorOccupied[m].second +2)> colorOccupied[m+1].first){
            //verify
          // cout<<"condition ms2, "<<m<<endl;
            ...
            //verify, no merge in this iteration
            cout<<"condition ms3, "<<m<<endl;
            m+=1;
        }
    }//end of while loop
    ...
    if(colorOccupied.size()>1){
        for (int i=0;i<(colorOccupied.size()-1);i++){
            ...
            cout<<colorOccupied[i].second<<"\n"<<colorOccupied[i+1].first<<endl;
            }
        }
    }
...
pair<size_t,size_t> FFallocation(vector<pair<size_t,size_t>> colorMerged,size_t size, size_t local_offset){
    ...
    if (colorMerged.size()>1) {
        ...
        // condition D: allocate after the last block.--> 0
        if (yLocation == -1){
...
//    int location ;
    ...
    map<void*,int>Table_p2r; //ptr for arrival idx
};
...
//        cout<<"============print tables done==========="<<endl;
    ...
    gc++;
    Table_p2s[allocatedPtr]=size;//TODO(junzhe) to remove
    ...
    if (loadFlag=="printVertices"){
        ...
        for (int i=0; i<188;i++){
            if(!(Table_r2Ver.find(i)==Table_r2Ver.end())){
                ...
                if(Table_r2Ver.find(i)->second.size==358312)
