...
///below shows in memInfo_2_sortedDict
pair<vector<onePairMsg>,vector<onePairMsg>> memInfo_2_sortedPair(string fileName){
...
vector<Vertex> run(vector<onePairMsg> pairMsgVec_, int &offset){
    int local_offset = offset; //TODO(junzhe) verify, this shall never be changed. to be feed to FFallocation.
...
//    cout<<temp4.first<<' '<<temp4.second<<endl;
    ...
    pair<vector<onePairMsg>,vector<onePairMsg>>pairOfPair = memInfo_2_sortedPair(fileName);
    vector<onePairMsg>pairMsgVec_1 = pairOfPair.first;
    vector<onePairMsg>pairMsgVec_2 = pairOfPair.second;
    cout<<"no testing, size of onePair 1: "<<pairOfPair.first.size()<<endl;
    cout<<"no testing, size of onePair 1: "<<pairMsgVec_1.size()<<endl;
    cout<<"no testing, size of onePair 2: "<<pairMsgVec_2.size()<<endl;
    //TODO(junzhe) no to do second
    int offset = 0;
    vector<Vertex>vertices_2 = run(pairMsgVec_2,offset);
    cout<<"offset after first round: "<<offset<<endl;
    vector<Vertex>vertices = run(pairMsgVec_1,offset);
    vector<Vertex>vertices_1 =vertices; //for computing maxload.
    cout<<"new offset, aka maxMemoryFootprint: "<<offset<<endl;
    //cout<<vertices[10].size<<' '<<vertices[10].colorRange.second-vertices[10].colorRange.first+1<<endl;
    cout<<"size of vertices_2, vertices :"<<vertices_2.size()<<' '<<vertices.size()<<endl;
    ...
    // returning the same value as py version maxload.
    // time complexity O(n^2), to be optimzied.
    //TODO(junzhe) taking max, and maxIdx declare locations.
    int maxTime =10000; //use maxIdx TODO(junzhe)
...
//        cout<<load[i]<<endl;
    //below maxload is based on some of vertices 1 and 2:
    ...
    string fileName2 ="memInfo_alex_20itr_100size.text";
...
//    }
    ...
    //cout<<"below test why some 2nd iteration cannot make it"<<endl; TODO
