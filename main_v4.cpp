...
struct lookUpElement{
    ...
    int size;
    int offset;
    void* ptr; //TODO(junzhe) to see if void* or string better
...
public:
    SmartMemPool();
    //TODO(junzhe) change int to size_t
    //TODO(junzhe) in Singa, void Malloc( void**, size_t)
...
private:
    ...
    int globeCounter;
    int tableReady ; //if look up table is done or not.
    //TODO(junzhe) private variables
    vector<string> vec;
    void* ptrPool;
    int idxRange ;
    int offset ;
...
SmartMemPool::SmartMemPool(){
    ...
    int globeCounter=-1;
    int tableReady =0;//if look up table is done or not.
    void* ptrPool = NULL;
    int idxRange = 0;
    int offset = 0;
    int maxload = 0;
...
void* SmartMemPool::Malloc(int size){
    ...
    if (gc == globeCounter){
        ...
        //move redundant Table1 element, but not necessary. idxRange. TODO(junzhe)
        //verify Table1 and Table2 TODO(junzhe)
        
        //update ptrPool
        ptrPool = malloc((size_t)(offset+1)); //TODO(junzhe) verify if +1.
    }
    
    if(mallocFlag==0){
        ///  2. if flag=0, malloc/cudaMalloc
        
        //TODO(junzhe) in singa
    ...
    while ((gc%100==0) && (mallocFlag==0) && (globeCounter==-1)){
        ///4. test repeated sequence every 100 blocks, update globeCounter.
...
//    cout<<ptr<<endl;
    ...
    char* temp10 = (char*)ptr+2*sizeof(char);
    cout<<(void*)temp10<<endl;
